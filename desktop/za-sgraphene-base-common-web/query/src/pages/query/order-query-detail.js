...
class OrderQueryDetail extends React.Component {
    super(props) {
        this.props = props;
    }
    state = {
    ...
    requestPolicyDetail = () => {
        ...
        T.fetch({
            ...
            if (res.success) {
                this.setState({
                    issuanceDetail: res.value || {}
                })

                // 保障责任列表
                let dutyList = []
                res.value.issuanceProductEsDTOList.map(item => {
                    item.issuanceProductLiabilityList.map((group, index) => {
                ...
                let insurantList = []
                let beneficiaryList = []
                res.value.issuanceProductEsDTOList.map((item, index) => {
                    item.issuanceInsurantEsList.map((group, ind) => {
    ...
    render() {
        ...
        const { 
            ...
            underwriteDecision,
            issuanceHolderEsDTO
        } = this.state.issuanceDetail
        const policyPayerList = this.state.issuanceDetail.issuancePayerList || []
            ...
            <div className="card-container policy-query-detail">
                <Tabs type="card">
                    <TabPane tab="保单信息" key="1">
                        <Row className="pb10">
                            <Col span={8}><span className="fb">订单编号:</span><span className="ml15">{orderNo}</span></Col>
                        ...
                        <Row className="pb10">
                            ...
                            <Col span={8}><span className="fb">状态:</span><span className="ml15">{issuanceStatus}</span></Col>
                        </Row>
                        <Row className="pb10">
                    ...
                    <TabPane tab="用户信息" key="2">
                        ...
                        <Row className="pb10 pt10">
                            <Col span={8}><span className="fb">姓名:</span><span className="ml15">{issuanceHolderEsDTO ? issuanceHolderEsDTO.customerDetailInfoDTO.name : ''}</span></Col>
                            <Col span={8}><span className="fb">证件类型:</span><span className="ml15">{issuanceHolderEsDTO ? issuanceHolderEsDTO.certiTypeName : ''}</span></Col>
                            <Col span={8}><span className="fb">证件号码:</span><span className="ml15">{issuanceHolderEsDTO ? issuanceHolderEsDTO.customerDetailInfoDTO.certiNo : ''}</span></Col>
                        </Row>
                        <Row className="pb10">
                            <Col span={8}><span className="fb">邮箱:</span><span className="ml15">{issuanceHolderEsDTO ? issuanceHolderEsDTO.customerDetailInfoDTO.emails[0].email : ''}</span></Col>
                            <Col span={8}><span className="fb">手机号码:</span><span className="ml15">{issuanceHolderEsDTO ? issuanceHolderEsDTO.customerDetailInfoDTO.phones[0].phoneNo : ''}</span></Col>
                            <Col span={8}><span className="fb">性别:</span><span className="ml15">{issuanceHolderEsDTO ? issuanceHolderEsDTO.genderName : ''}</span></Col>
                        </Row>
                        <Row className="pb10">
                            <Col span={8}><span className="fb">出生日期:</span><span className="ml15">{issuanceHolderEsDTO ?  moment(issuanceHolderEsDTO.customerDetailInfoDTO.birthday).format('YYYY-MM-DD') : ''}</span></Col>
                        </Row>
...
export default OrderQueryDetail;