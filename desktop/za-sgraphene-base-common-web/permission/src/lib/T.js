...
var T = window.T = {
	...
	showError(msg) {
		message.error(msg || window.LangMessage.message_error);
	},
	showWarn(msg) {
		message.warning(msg);
	},
	...
	fetch(opt) {
		...
		return new Promise(function (resoved, reject) {
			fetch(url, param).then((res) => {
				...
				json.then((result) => {
                    if (result.code === '101') {
                        if (self != top) window.parent.goNsso();
                        // console.log('login timeout');
                        // let currentURL = window.location.protocol + '//' + window.location.host;
						// window.location.href = "/sso/login.html?service=productCenter&target=" + encodeURIComponent(currentURL);
						T.fetch({
							url: "/logoutRequest"
						}).then((result) => {
							if (result.success) {
					...
					if (!result.success) {
						T.showError(result.errorMsg || result.msg);
					}
					resoved(result);
				});
			});
